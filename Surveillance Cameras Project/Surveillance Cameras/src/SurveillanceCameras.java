...
public class SurveillanceCameras {
		...
		while(true) {
			...
			switch(choice) {
				case 1: 		Nsolution = ob.greedySearch();
							...
							System.out.println("Path Cost: "+ cameras.length/2);
				...
				case 2:		Nsolution = ob.A();
							...
							System.out.println("Path Cost: "+ Nsolution.cost);
							System.out.println("Solution Path: ");
							while(!path.isEmpty()) {
								printM(ob.visibleAreas(path.removeLast()),roomSize,roomSize);
								System.out.println("\n-----------------------------");
				...
				case 3: neighbors = ob.hillClimbing(100);
						...
						while(!neighbors.isEmpty()) {
							...
							System.out.println("Neighbor number ["+counter+"] objective function= "+ ob.objectiveFunction(solution));
							counter++;
				...
				case 4: neighbors = ob.simulatedAnnealing(100);
						solution = ob.visibleAreas(neighbors.removeLast());
						System.out.println("Best found solution objective function= "+ ob.objectiveFunction(solution));
						printM(solution,roomSize,roomSize);
						System.out.println("\n-----------------------------");
	...
	public LinkedList<int[]> simulatedAnnealing(int maxIteration) {
		...
		double temp= roomSize*roomSize*roomSize; //starting temperature
